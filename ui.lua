...
function Library:create(options)
	...
	if readfile and writefile and isfile then
		if not isfile("GalaxySettings.json") then
			writefile("GalaxySettings.json", HTTPService:JSONEncode(settings))
		end
		settings = HTTPService:JSONDecode(readfile("GalaxySettings.json"))
	...
	options = self:set_defaults({
		...
		Link = "https://github.com/Craexz/cool-ui/"
	}, options)

	if getgenv and getgenv().GalaxyUI then
		getgenv():GalaxyUI()
		getgenv().GalaxyUI = nil
	end
	...
	creditsTab:credit{Name = "Mortal", Description = "UI Library Developer", Discord = "mortal#9121"}
